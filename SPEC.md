...
`zwill twin-benchmark run --config <config.json>` runs a set of digital twin studies from a JSON config. The config contains benchmark metadata, optional default sampling/model settings, and a `studies` list. Each study must name a `survey` and one or more held-out questions. The command should call the same `twin-study run` machinery for each study, then write a benchmark manifest containing survey names, job ids, paths, and run status. `--dry-run` must export jobs without model calls.
