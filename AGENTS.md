...
- Run EDSL model jobs only with an environment configured for remote Expected Parrot inference. Before any `zwill edsl-run`, verify the remote EP profile/key is available, then use the plain wrapper command so it calls EDSL `.run()` normally. Do not inject `use_api_proxy`, `disable_remote_inference`, `offload_execution`, or other run parameters unless the user explicitly asks for them. If remote execution cannot be verified or fails because credentials/profile are missing, stop and report the blocker with the exact resume command instead of trying a local run or direct local provider calls.
...
7. **Export, run, and import twin jobs from the approved plan.**
   ...
   Run each exported approach job from an environment configured for remote Expected Parrot inference. If the package is handed to another runner, use its `RUN.md`. The local command wrapper pattern is plain `zwill edsl-run`, which should call EDSL `.run()` normally without injecting proxy, offload, or local/remote override parameters:
